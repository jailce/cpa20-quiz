...
function loadPage() {
	...
	if (status == "not attempted") {
		...
		//doLMSSetValue("cmi.core.score.min","70");
		//doLMSSetValue("cmi.suspend_data","0");
...
function salvaPagina(p_pagina) {
	console.log(p_pagina)
 	p_pagina = p_pagina.toString()
  	doLMSSetValue('cmi.core.lesson_location', p_pagina)
  	doLMSCommit()
  	console.log(p_pagina)
}
...
function fl_finaliza(notaFinal){
	notaFinal = parseFloat(notaFinal)

	if(notaFinal < 70){
		var aprovacao = "failed"
		doLMSSetValue("cmi.core.lesson_status",aprovacao);
		doLMSCommit();

	}else if(notaFinal > 70){
		var aprovacao = "passed"
		doLMSSetValue("cmi.core.lesson_status",aprovacao);
		doLMSCommit();
...
function fl_getLessonLocation(){
	console.log(doLMSGetValue("cmi.core.lesson_location"))	
